...
# ╔═╡ bc8cf884-177a-11f1-a74b-078bcb904ed8
using Symbolics, ModelingToolkit, DifferentialEquations, Plots

# ╔═╡ 25d94975-58cc-4586-bbd4-8848d991fd7d
import Latexify

# ╔═╡ bf3b2022-4a29-458c-9de0-73230bc087de
default(fontfamily = "Computer Modern", linewidth = 2, size = (800, 600))

# ╔═╡ e0281594-c67c-4287-bbdd-854199778782
...
# ╔═╡ f62d1430-618a-48f3-92ec-5042f60f66fd
r_rot = [L*sin(θ)*cos(φ), L*sin(θ)*sin(φ), -L*cos(θ)]

# ╔═╡ 0a041946-cc15-4fb4-bb4e-009c8f970929
r_0 = [w1*cos(φ), w1*sin(φ), h1]

# ╔═╡ 6b86be7d-57cc-40f4-ad85-9628dda7952f
r = r_rot .+ r_0

# ╔═╡ 9264cc9c-60bd-41a8-bf9b-65e058be0bfc
# defining lagrangian
begin
	h = r[3]
	v = D.(r)
...
# ╔═╡ 4b9bedec-2c7b-44a7-83a1-bc58fe08745c
...
# ╔═╡ 4d6d58db-7550-4ee3-a3e0-cb87bc5f0020
...
# ╔═╡ 5198a4a3-839c-432a-b1ee-e316a78fcc5a
# ****CHANGEDDDD****: Running the Super-Critical Simulation
begin
    # We use a tiny 0.01 offset so it 'falls' away from the center
    u0_super = [x1 => 0.01, x2 => 0.0]   
    t_span_super = (0.0, 20.0) 

    # Ω = 12.0 is the "Magic Speed" where it stays tilted
    # c = 0.01 is the damping that helps it settle at that angle
    p_super = Dict(L => 0.15, g => 9.8, Ω => 12.0, m => 0.1, w1 => 0.1, h1 => 0.2, c => 0.01)

    prob_super = ODEProblem(sys, u0_super, t_span_super, p_super)
    
    # sol_super is now your main result
    sol_super = solve(prob_super, Tsit5(); reltol=1e-8, abstol=1e-8, saveat=0.0333)
end

# ╔═╡ a3e67148-14d7-447c-8c02-79f036389a0e
...
# ╔═╡ 09cf4289-bd72-4ffd-b208-94ea5b89bce0
begin
	anim_super = create_pendulum_animation(sol_super, p_super, p_super, "Ω = 12.0 rad/s")
    gif(anim_super, "pendulum_super_critical.gif", fps=30)
end

# ╔═╡ dd2043ae-24db-4844-8324-cc6b2560fe1b
#gif(anim_slow, "visuals/pendulum_slow_undamped.gif", fps=30)
gif(anim_super, "pendulum_slow_undamped.gif", fps=30)

# ╔═╡ 62c06cdf-cedf-4a85-8040-0df10433ee23
#gif(anim_fast, "visuals/pendulum_fast_undamped.gif", fps=30)
gif(anim_super, "pendulum_fast_undamped.gif", fps=30)

# ╔═╡ 535bdb29-ae32-4e15-9b78-3b16fd515690


# ╔═╡ 4d777410-8f95-4426-b959-04785d877173


# ╔═╡ ed6c446a-407a-494e-bdee-e898829d80a1


# ╔═╡ 6ae21cb2-62eb-422a-a112-575f2ea9f3d5


# ╔═╡ 7fb42da0-f01c-48d7-bd1a-f8b002caef63


# ╔═╡ 74274b93-cb15-4c45-ac61-7006c45fa170
